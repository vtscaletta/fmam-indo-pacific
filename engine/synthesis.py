...
# --- Коэффициенты синтез-формулы ------------------------------------------
#
# Коэффициенты не подбираются. Они выводятся из двух условий, вследствие чего
# ни один из критериев проверки не превращается в цель настройки.
#
# Условие первое, равные доли. Три составляющие материального контура, а
# именно расходы, риторика и институциональный дрейф, входят равными долями,
# поскольку теоретическая рамка указывает на существенность всех трёх и не
# указывает, во сколько раз одна существеннее другой. Материальный и
# перцептивный контуры входят равными половинами по тому же основанию.
#
# Условие второе, середина шкалы. Система, у которой все переменные стоят
# посередине области определения, обязана давать напряжение посередине. Из
# этого условия смещение определяется однозначно.
#
# Остаётся крутизна отклика. Она задаётся требованием, чтобы теоретически
# достижимый размах подлогистического выражения покрывал рабочий участок
# функции без насыщения на концах. Выражение при равных долях лежит в
# пределах от минус половины до половины, вследствие чего крутизна шесть
# даёт размах аргумента от минус трёх до трёх и напряжение от 0,047 до 0,953.

STEEPNESS = 6.0
"""Крутизна отклика напряжения. Единственная величина слоя, заданная правилом."""


def beta_from_rule(steepness: float = STEEPNESS) -> dict:
    """
    Коэффициенты синтез-формулы, выведенные из равных долей и середины шкалы.

    Возвращает смещение и четыре веса. Проверяется тождеством, что при всех
    переменных, равных половине, напряжение равно половине.
    """
    k = float(steepness)
    return {
        "b0": -k / 2.0,
