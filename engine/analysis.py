...
def classify(trajectory, thresholds: dict) -> dict:
    ...
    parts = [f"К концу горизонта риск дестабилизации составляет {risk * 100:.0f}%"]
    if breaches:
        parts.append(f"в пике напряжение пробивает порог срыва {th23:.3f}")
    else:
        parts.append(f"пик напряжения {peak:.3f} ниже порога срыва {th23:.3f}")
    if level == "red":
        parts.append("обстановка требует оперативного решения, долгосрочные стратегии вторичны")
    elif level == "amber":
        parts.append("устойчивая конфронтация без открытого срыва, нужен присмотр")
    else:
        parts.append("силы уравновешены, допустимы долгосрочные стратегии")
