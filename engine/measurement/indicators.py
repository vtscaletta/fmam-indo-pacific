...
class Kind(str, Enum):
    ...
    SHARE_DEV    та же доля, взятая отклонением от собственного обычного
                 уровня пары. Постоянная асимметрия сил не означает
                 постоянной угрозы, вследствие чего восприятие измеряется
                 отклонением от привычного положения
